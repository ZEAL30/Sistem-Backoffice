...
<form method="POST" action="{{ route('product.store') }}" enctype="multipart/form-data" style="display:grid; grid-template-columns: 1fr 1fr; gap:24px; margin-bottom:30px;">
    @csrf

    <!-- LEFT COLUMN: FORM INPUT -->
    <div>
        <div style="background:#fff; border-radius:12px; padding:24px; box-shadow: 0 6px 20px rgba(37,44,69,0.04);">
            <h2 style="color:#252C45; font-size:20px; margin:0 0 20px 0; font-weight:700;">Tambah Produk</h2>
        ...
        <div style="background:#fff; border-radius:12px; padding:24px; box-shadow: 0 6px 20px rgba(37,44,69,0.04); margin-bottom:20px;">
            ...
            <div style="max-height:200px; overflow-y:auto;">
                @foreach ($categories as $category)
                    <label style="display:flex; align-items:center; gap:8px; margin-bottom:10px; cursor:pointer; color:#666;">
            ...
            @error('categories') <p style="color:red; font-size:12px; margin:8px 0 0 0;">{{ $message }}</p> @enderror
        </div>
        ...
        <div style="background:#fff; border-radius:12px; padding:24px; box-shadow: 0 6px 20px rgba(37,44,69,0.04);">
            ...
            <input type="text" id="slug-display" placeholder="slug-produk"
...
document.querySelector('form[action="{{ route("product.store") }}"]').addEventListener('submit', function(e) {
    ...
    if (!featuredImageValue) {
        console.warn('Warning: featured_image is empty on form submit');
    }

    // Debug: log full FormData to verify categories[] and other fields are submitted
    try {
        ...
        console.group('FormData dump on submit');
        console.log(entries);
        const categories = formData.getAll('categories[]');
        console.log('categories[]', categories);
        console.groupEnd();
    } catch (err) {
        console.warn('Could not read FormData for debug:', err);
    }
});
