...
<form method="POST" action="{{ route('post.store') }}" enctype="multipart/form-data" style="display:grid; grid-template-columns: 1fr 1fr; gap:24px; margin-bottom:30px;">
    @csrf

    <!-- LEFT COLUMN: FORM INPUT -->
    <div>
        <div style="background:#fff; border-radius:12px; padding:24px; box-shadow: 0 6px 20px rgba(37,44,69,0.04);">
            <h2 style="color:#252C45; font-size:20px; margin:0 0 20px 0; font-weight:700;">Tambah Artikel</h2>
            ...
            @endif
                ...
                <div style="margin-bottom:18px;">
                    ...
                    <textarea name="excerpt" style="width:100%; padding:10px 12px; border:1px solid #ddd; border-radius:8px; font-size:14px; font-family:inherit; resize:vertical; min-height:80px;"
                        placeholder="Masukkan ringkasan artikel">{{ old('excerpt') }}</textarea>
                ...
                <div style="margin-bottom:18px;">
                    ...
                    <textarea name="content" style="width:100%; padding:10px 12px; border:1px solid #ddd; border-radius:8px; font-size:14px; font-family:inherit; resize:vertical; min-height:150px;"
                        placeholder="Masukkan isi artikel">{{ old('content') }}</textarea>
                ...
                <div style="margin-bottom:20px;">
                    <label style="display:flex; align-items:center; gap:8px; cursor:pointer; color:#252C45; font-weight:600; font-size:14px;">
                        <input type="checkbox" id="status_checkbox" onchange="setStatus(this)" {{ old('status') == 'published' ? 'checked' : '' }}>
                        <span>Publikasikan Artikel</span>
                    </label>
                    <input type="hidden" name="status" id="status" value="{{ old('status', 'draft') }}">
                </div>
    ...
    <div>
        ...
        <div style="background:#fff; border-radius:12px; padding:24px; box-shadow: 0 6px 20px rgba(37,44,69,0.04);">
            ...
            <input type="text" id="slug-display" placeholder="slug-artikel"
...
document.querySelector('form[action="{{ route("post.store") }}"]').addEventListener('submit', function(e) {
